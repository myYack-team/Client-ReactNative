...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .arm-action-group {
    transform-origin: 118px 123px;
    animation: armActionSequence 8s linear infinite;
  }
...
const SVG_WIDTH = Math.min(SCREEN_WIDTH - 48, 320);
const SVG_HEIGHT = SVG_WIDTH * 2; // 1:2 비율

export function AnalysisLoadingModal({ visible }: AnalysisLoadingModalProps) {
  const [currentStage, setCurrentStage] = useState(0);
  const [progress, setProgress] = useState(0);

  // 단계 진행 (8초 주기 = SVG 애니메이션과 동기화)
  useEffect(() => {
    if (!visible) {
      setCurrentStage(0);
      setProgress(0);
    ...
    const progressInterval = setInterval(() => {
      setProgress(prev => {
        const nextProgress = prev + 0.15;
        return Math.min(nextProgress, 99);
      });
    }, 100);

    // 단계 변경 (2초 간격 - 8초 애니메이션의 4단계)
